...
std::shared_ptr<WebServSession> WebServSessionList::getSession(std::string sessionId) {
	auto it = std::find_if(_sessions.begin(), _sessions.end(), [sessionId](const WebServSession& session) {
		return session.getSessionId() == sessionId;
	});
	if (it == _sessions.end())
		throw std::runtime_error("Session not found");
	return std::make_shared<WebServSession>(*it);
}

std::shared_ptr<WebServSession> WebServSessionList::createSession() {
	_sessions.push_back(WebServSession());
	return std::make_shared<WebServSession>(_sessions.back());
}

void WebServSessionList::removeSession(std::string sessionId) {
	_sessions.erase(std::remove_if(_sessions.begin(), _sessions.end(), [sessionId](const WebServSession& session) {
		return session.getSessionId() == sessionId;
	}), _sessions.end());
}

void WebServSessionList::removeExpiredSessions(std::chrono::time_point<std::chrono::steady_clock> now) {
	_sessions.erase(std::remove_if(_sessions.begin(), _sessions.end(), [now,this](const WebServSession& session) {
		bool expired = (now - session._lastActivityTime) > std::chrono::seconds(DEFAULT_TIMEOUT_SECONDS);
		if (expired)
			infoLog << "Session expired: " << session.getSessionId() << " session list length before deletion: " << this->_sessions.size() << CPPLog::end;
		return (expired);
	}), _sessions.end());
}
