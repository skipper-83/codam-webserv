...
Environment::Environment(const std::map<std::string, std::string> &environment) : _env(nullptr) {
    try {
        _env = new char *[environment.size() + 1](nullptr);
...
Environment &Environment::operator=(const Environment &other) {
    ...
    this->_env = new char *[length](nullptr);
...
AsyncProgram::AsyncProgram(const std::string &exec, const std::map<std::string, std::string> &environment, const ProgramCallback &programReadReadyCb,
    ...
    if (pid == 0) {
        ...
        char *const args[2] = {const_cast<char *>(exec.c_str()), nullptr};
...
std::unique_ptr<AsyncProgram> AsyncProgram::create(const std::string &exec, const std::map<std::string, std::string> &environment,
    ...
    logD << "AsyncProgram::create(const std::string&, const std::map<std::string, std::string>&, const ProgramCallback&, const ProgramCallback&) "
            "called";
    return std::make_unique<AsyncProgram>(exec, environment, programReadReadyCb, programWriteReadyCb);
}
