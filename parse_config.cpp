...
std::istream& operator>>(std::istream& is, ServerNames& lhs) {
	...
	getline(is, line);
	if (line[line.size() - 1] != ';')
		throw(std::invalid_argument("Missing terminator ; for server_name"));
	lineStream.str(line);
...
void	setLocationRoot(std::istream &is, Location& lhs)
{
	std::string word;
...
void	setLocationIndex(std::istream &is, Location &lhs)
{
	...
	if (line[line.length() - 1] != ';')
		throw(std::invalid_argument("Missing terminator ; for index list in location"));
